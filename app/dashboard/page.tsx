...
export default function DashboardPage() {
  ...
  const openCreateFlavor = () => {
    setFlavorForm({ slug: '', description: '' })
    setError('')
    setFlavorModal('create')
  }

  const openEditFlavor = (f: Flavor) => {
    setFlavorForm({ slug: f.slug, description: f.description })
    setError('')
    setFlavorModal('edit')
  }

  const openDeleteFlavor = () => {
    setError('')
    setFlavorModal('delete')
  }
  ...
  const saveStep = async () => {
    ...
    const payload: any = {
      ...
      humor_flavor_step_type_id: stepForm.humor_flavor_step_type_id
        ? Number(stepForm.humor_flavor_step_type_id)
        : 1,
